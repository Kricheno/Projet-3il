...
public class PowerShellTest {
    ...
    private static final org.apache.logging.log4j.Logger l=  LogManager.getLogger(PowerShellTest.class);
    ...
    @Test
    public void projetParamsTest() {
        l.info("testProjetParams");
        ...
        if (OSDetector.isWindows()) {
            ...
            //PowerShellResponse response = powerShell.executeCommand("wmic /NODE: 192.168.0.17 /user: Projet_3iL /password: omartest path Win32_Keyboard get Description /value | find /i /c --% \"keyboard\"");
            PowerShellResponse responseClavier = powerShell.executeCommand("wmic /node:\""+poste.getAdresse_IP()+"\" /user:\""+user+"\" /password:  \""+password+"\" path Win32_Keyboard get Description /value | find /i /c --% \"Description\"");

            //Pour detecter la souris
            PowerShellResponse responseSouris = powerShell.executeCommand("wmic /node:\""+poste.getAdresse_IP()+"\" /user:\""+user+"\" /password:  \""+password+"\" path Win32_PointingDevice get Name /value | find /i /c --% \"Name\"");
            //Pour detecter la ecran
            PowerShellResponse responseEcran = powerShell.executeCommand("wmic /node:\""+poste.getAdresse_IP()+"\" /user:\""+user+"\"  /password:  \""+password+"\" get-ciminstance -namespace root/wmi -classname WmiMonitorConnectionParams | find /i /c --% \"DISPLAY\"");
...
//          Stream<Integer> stream= Stream.of(nbreClavier,nbreEcran,nbreSouris);
//
//            stream.forEach((Integer i)->l.info("il y'a "+i));
            //Message
            if( nbreClavier>=1 ) {
                l.info("il y a " + nbreClavier + " clavier");
            }
            else {                 l.info("Aucun clavier n'a été détecté");
            }
            if( nbreSouris>=1) {
                l.info("il y a " + resultatSouris + " souris");
            }
            else {                 l.info("Aucun souris n'a été détecté");
            }
            if( nbreEcran>=1) {
                l.info("il y a " + resultatEcran + " ecran");
            }
    ...
    @Test
    public void projetTest() {
        System.out.println("testProjet");
        if (OSDetector.isWindows()) {
            PowerShell powerShell = PowerShell.openSession();
            // Pour detecter le clavier
            //PowerShellResponse response = powerShell.executeCommand("wmic /NODE: 192.168.0.17 /user: Projet_3iL /password: omartest path Win32_Keyboard get Description /value | find /i /c --% \"keyboard\"");
